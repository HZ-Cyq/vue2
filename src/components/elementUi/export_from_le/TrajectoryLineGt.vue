  ...
  <div class="missile-visualization">
    ...
    <div class="select-group">
      <select v-model="selectedIndex">
        <option v-for="(item, index) in ljdetails" :key="index" :value="index">
...
export default {
  ...
  computed: {
    currentData() {
      ...
      if (this.ljdetails && this.ljdetails.length > 0) {
        return this.ljdetails[this.selectedIndex] || this.ljdetails[0];
      }
      return null;
    }
  },
  ...
  watch: {
    ...
    selectedIndex() {
      if (!this.trajChart || !this.ganttChart) return;
      this.calculateTimeRange();
      this.renderCharts();
    }
  },
